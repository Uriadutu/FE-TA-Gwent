...
const EditLabelModal = ({ setIsOpenModalEdit, selectedLabel, getLabel }) => {
  const [namaLabel, setNamaLabel] = useState("");
  const [loading, setLoading] = useState(false);

  useEffect(() => {
    if (selectedLabel) {
      setNamaLabel(selectedLabel.nama_label || "");
    }
  }, [selectedLabel]);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      await axios.put(`http://localhost:8000/label/${selectedLabel.id}`, {
